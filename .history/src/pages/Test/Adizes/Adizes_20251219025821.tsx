...
export function AdizesPage() {
  ...
  useEffect(() => {
    ...
    // Только если явно перешли из профиля с сохраненным результатом
    if (state?.showSavedResult === true && state?.savedResult) {
      ...
      // Если нет сохраненного результата, гарантируем что showResults = false
      // Это позволяет вопросам показываться при обычном прохождении теста
  ...
  const isAllBlocksCompleted = currentBlock === totalBlocks && Object.keys(currentBlockAnswers).length > 0;
  ...
  return (
    <div className="adizes-page">
        ...
        <div className="adizes-content">
          ...
          {!showResults && (
            ...
            <p className="adizes-intro">
              В основі методики лежить <strong>PAEI-концепція</strong>, згідно з якою кожна людина має <strong>унікальний набір якостей</strong>
              (відрізняється темпераментом, має свої особливості поведінки, стиль роботи, лідерські здібності, сильні та
              слабкі сторони), знаючи які, можна визначити свій <strong>індивідуальний стиль менеджменту</strong>.
            </p>
          </section>
          )}

          {/* PAEI Dimensions Section */}
          {!showResults && (
            <section className={`adizes-dimensions relative ${isProducerHidden ? 'adizes-dimensions--hidden' : ''}`}>
            <button
              type="button"
                  ...
                  <div key={key} className="adizes-dimension-card">
                    <div className="adizes-dimension-header max-w-[595px] w-full flex justify-between relative">
                      ...
                      <div className="adizes-dimension-content">
                        ...
                        <p className="text-[#262626] font-montserrat font-[500] leading-6 mt-[10px]">{dimension.description}</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {!showResults && (
              ...
              <div className="adizes-instructions-box">
                ...
                <div className="adizes-instructions-content">
                  <p className="font-medium mb-[10px] leading-[20.8px] text-justify">
                    Перед вами 10 блоків, що містять по 4 якості особистості. Надайте кожній якості від 1-го до 4-х балів
                    в залежності від того, наскільки воно підходить саме вам. Загальна сума балів одного блоку повинна
                    дорівнювати 10.
                  </p>
            ...
            <div className="adizes-robot">
              <img src={robotImageUrl} alt="Robot" width={320} height={320} className="w-[320px] h-[320px] object-contain" />
            </div>
          </section>
          )}
          ...
          {!showResults && (
            ...
            <div className="adizes-questions-main">
              ...
              <div className="flex items-center justify-start md:justify-end gap-[50px] max-w-[545px] w-full pl-[22px] md:pl-0 md:pr-[19px] mb-[12px]">
                {[1, 2, 3, 4].map((num) => (
                  <div key={num} className="text-[#5D5D5D] font-[600] text-[16px] font-montserrat">
                    {num}
                  </div>
                ))}
              </div>
              ...
              <div className="adizes-questions-list">
                {currentBlockData?.questions?.map((question, questionIndex) => (
                  <div key={questionIndex} className="min-w-[200px] w-full md:w-[545px] flex flex-col gap-[10px] md:gap-[20px]">
                    <p className="md:hidden text-[14px] text-[#262626] font-unbounded font-[400]">{question}</p>
                    <div className="flex justify-between items-center py-[14px] px-[19px] bg-white rounded-[5px]">
                      <p className="hidden md:block text-[#262626] font-unbounded font-[400]">{question}</p>
                        ...
                        {[1, 2, 3, 4].map((value) => (
                          <button
                            ...
                            className={`w-[10px] h-[10px] rounded-[100%] duration-500 ${
                              currentBlockAnswers[questionIndex] === value ? 'bg-[#347AEC]' : 'bg-[#E0E0E0]'
                            }`}
          ...
          {!showResults && (
            <div className="adizes-navigation">
              {currentBlock > 1 && (
                <button type="button" className="adizes-nav-button adizes-nav-button--prev" onClick={handlePrevBlock}>
                  ← Попередній блок
                </button>
              )}
              {currentBlock < totalBlocks && (
          ...
          {isAllBlocksCompleted && !showResults && (
            <div className="adizes-get-result-section">
              <button
                type="button"
                onClick={handleGetResult}
                className="adizes-get-result-button"
              >
          ...
          {showResults && (() => {
            ...
            if (isShowingSavedResult) {
              ...
              const dominantType = 
                results.P === maxScore ? 'P' :
                results.A === maxScore ? 'A' :
                results.E === maxScore ? 'E' : 'I';
            ...
            const description = getDescriptionForType(dominantType as 'P' | 'A' | 'E' | 'I', results[dominantType as 'P' | 'A' | 'E' | 'I']);
              ...
              <>
                  ...
                  <div className="adizes-result-image-container">
                    <div className="adizes-result-image-card">
                      <div className="adizes-result-image-labels">
                        ...
                        <div className="adizes-result-image-label-bottom">{dominantData.ukrainianTitle}</div>
                      </div>
                      <img 
                        src={resultImageUrl} 
                ...
                {!isShowingSavedResult && (
                  ...
                  >
                    ...
                    {shouldShowAsSaved && (
                      <p className="adizes-save-message">Результат збережено в особистому кабінеті</p>
                    )}
                  </div>
                )}
              </div>
      ...
      {showLeaveModal && (
        <div className="adizes-modal-overlay" onClick={() => {
