...
export function Sidebar() {
  ...
  useEffect(() => {
    const checkMobile = () => {
      ...
      if (!mobile) {
        // Для десктопа сайдбар всегда sticky, добавляем класс для тени при прокрутке
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        // Не удаляем токен при ошибке, возможно это временная проблема
      } finally {
        setIsLoading(false);
      }
    };
    ...
    // Слушаем событие обновления пользователя (когда загружается аватар в профиле)
  ...
  // Показываем имя пользователя, если оно есть, иначе показываем "Користувач" только если пользователь авторизован
  const userName = userData?.first_name || (isAuthenticated ? 'Користувач' : 'Anonymous');
  ...
  const handleNavigation = (href: string) => {
    ...
    const isTestPage = 
      location.pathname.includes('/test/enneagram') ||
      location.pathname.includes('/test/mbi') ||
      location.pathname.includes('/test/mbti') ||
      location.pathname.includes('/test/adizes') ||
      location.pathname.includes('/test/innovation-potential') ||
      location.pathname.includes('/emotional-intelligence');
    
    if (isTestPage) {
      // Отправляем кастомное событие для перехвата навигации
      window.dispatchEvent(new CustomEvent('navigation-attempt', { detail: href }));
    } else {
      // Обычная навигация
      navigate(href);
    }
  ...
  return (
    <aside 
      className={`w-full md:w-[248px] md:h-[654px] flex flex-col items-center md:items-start gap-6 sidebar-always-sticky ${!isMobile && isSticky ? 'scrolled' : ''}`}
    >
      <div 
      ...
      <div className="w-full md:h-full bg-white rounded-[15px] md:rounded-[15px] px-6 py-6 flex flex-col gap-6">
        <div className="flex items-start gap-4">
          <div className="w-[67px] h-[67px] rounded-[16px] bg-gradient-to-br from-[#508FFF] to-[#6E63F5] flex items-center justify-center overflow-hidden">
            {userData?.image ? (
              <img
                src={userData.image}
                alt={userName}
          ...
          <div className="flex-1 flex flex-col">
            {isLoading ? (
              ...
              <div className="flex flex-col">
                <p className="font-montserrat font-[500] text-[10px] text-[#5E5E5D] mb-0">
                  {greeting},
                </p>
