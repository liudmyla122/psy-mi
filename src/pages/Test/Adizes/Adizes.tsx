...
// 12 блоков по 4 вопроса в каждом
// Порядок согласно ТЗ: P (красный), A (желтый), E (зеленый), I (синий)
// Но в отображении порядок может отличаться - важно что индекс соответствует типу
const testBlocks = [
  {
    id: 1,
    // Согласно ТЗ: Чуйний (I), Яскравий (E), Залучений (P), Той, що має сумніви (A)
    // Порядок в массиве: P, A, E, I (индекс 0=P, 1=A, 2=E, 3=I)
    questions: ['Залучений', 'Той, що має сумніви', 'Яскравий', 'Чуйний'],
    colors: ['#FF0000', '#FFD700', '#00FF00', '#0000FF'], // P, A, E, I
  },
  {
    id: 2,
    // Согласно ТЗ: Прямолінійний (P), Аналізуючий (A), Харизматичний (E), Чуттєвий (I)
    questions: ['Прямолінійний', 'Аналізуючий', 'Харизматичний', 'Чуттєвий'],
    colors: ['#FF0000', '#FFD700', '#00FF00', '#0000FF'], // P, A, E, I
  },
  {
    id: 3,
    // Согласно ТЗ: Спостерігаючий (A), Той, що робить (P), Думаючий (E), Співпереживаючий (I)
    questions: ['Той, що робить', 'Спостерігаючий', 'Думаючий', 'Співпереживаючий'],
    colors: ['#FF0000', '#FFD700', '#00FF00', '#0000FF'], // P, A, E, I
  },
  {
    id: 4,
    // Согласно ТЗ: Свідомий (P), Оцінюючий (A), Ризикований (E), Доброзичливий (I)
    questions: ['Свідомий', 'Оцінюючий', 'Ризикований', 'Доброзичливий'],
    colors: ['#FF0000', '#FFD700', '#00FF00', '#0000FF'], // P, A, E, I
  },
  {
    id: 5,
    // Согласно ТЗ: Результативний (P), Логічний (A), Допитливий (E), Інтуїтивний (I)
    questions: ['Результативний', 'Логічний', 'Допитливий', 'Інтуїтивний'],
    colors: ['#FF0000', '#FFD700', '#00FF00', '#0000FF'], // P, A, E, I
  },
  {
    id: 6,
    // Согласно ТЗ: Активний (P), Спостерігаючий (A), Абстрактний (E), Взаємодіючий (I)
    questions: ['Активний', 'Спостерігаючий', 'Абстрактний', 'Взаємодіючий'],
    colors: ['#FF0000', '#FFD700', '#00FF00', '#0000FF'], // P, A, E, I
  },
  {
    id: 7,
    // Согласно ТЗ: Прагматичний (P), Той, що розмірковує (A), Орієнтований на майбутнє (E), Теплий (I)
    questions: ['Прагматичний', 'Той, що розмірковує', 'Орієнтований на майбутнє', 'Теплий'],
    colors: ['#FF0000', '#FFD700', '#00FF00', '#0000FF'], // P, A, E, I
  },
  {
    id: 8,
    // Согласно ТЗ: Переможець (P), Спостерігаючий (A), Концептуальний (E), Той, хто прагне консенсусу (I)
    questions: ['Переможець', 'Спостерігаючий', 'Концептуальний', 'Той, хто прагне консенсусу'],
    colors: ['#FF0000', '#FFD700', '#00FF00', '#0000FF'], // P, A, E, I
  },
  {
    id: 9,
    // Согласно ТЗ: Раціональний (P), Стриманий (A), Генератор ідей (E), Обізнаний (I)
    questions: ['Раціональний', 'Стриманий', 'Генератор ідей', 'Обізнаний'],
    colors: ['#FF0000', '#FFD700', '#00FF00', '#0000FF'], // P, A, E, I
  },
  {
    id: 10,
    // Согласно ТЗ: Діловий (P), Акуратний (A), Яскравий (E), Приємний (I)
    questions: ['Діловий', 'Акуратний', 'Яскравий', 'Приємний'],
    colors: ['#FF0000', '#FFD700', '#00FF00', '#0000FF'], // P, A, E, I
  },
  {
    id: 11,
    // Согласно ТЗ: Енергійний (P), Організований (A), Творчий (E), Дружній (I)
    questions: ['Енергійний', 'Організований', 'Творчий', 'Дружній'],
    colors: ['#FF0000', '#FFD700', '#00FF00', '#0000FF'], // P, A, E, I
  },
  {
    id: 12,
    // Согласно ТЗ: Орієнтований на результат (P), Методичний (A), Виділяється (E), Інтегрований (I)
...
export function AdizesPage() {
  ...
  const handleAnswerChange = (questionIndex: number, value: number) => {
    setAnswers((prev) => {
      ...
      // Проверяем, что сумма не превысит 10 баллов
      const currentSum = Object.values(blockAnswers).reduce((sum, val) => sum + val, 0);
      const newSum = currentSum - (currentValue || 0) + value;
      
      if (newSum > totalPointsPerBlock) {
        alert(`Сума балів у блоці не повинна перевищувати ${totalPointsPerBlock} балів. Поточна сума: ${currentSum}, спроба додати: ${value}`);
        return prev;
      }
  ...
  const handleNextBlock = () => {
    // Без строгой валидации суми балів у блоці — просто переходим к следующему блоку
  ...
  const handleGetResult = () => {
    // Разрешаем получение результата без строгой валидации сум по блокам
    setShowResults(true);
    setTimeout(() => {
      if (resultsRef.current) {
        resultsRef.current.scrollIntoView({ behavior: 'smooth', block: 'start' });
  ...
  // Проверяем только, что пользователь в последнем блоке и есть хотя бы один ответ
            ...
            <section className="adizes-instructions">
                ...
                <div className="adizes-instructions-box">
                  ...
                  <div className="adizes-instructions-content">
                    <p className="font-medium mb-[10px] leading-[20.8px] text-justify">
                      <TypewriterWords 
                        text="Перед вами 12 блоків, що містять по 4 якості особистості. Надайте кожній якості від 1-го до 4-х балів в залежності від того, наскільки воно підходить саме вам. Бали в 1 блоці можуть мати різну варіативність, але не повинні перевищувати 10 балів сумарно."
                        delay={100}
                      />
                    </p>
                    <h3 className="font-bold">
                      <TypewriterWords 
                        text="Будьте чесні з собою"
                        delay={120}
                        startDelay={3500}
                      />
                    </h3>
              ...
              <div className="adizes-robot">
                <img
                  ...
                  width={320}
                  height={320}
                  className="w-[320px] h-[320px] object-contain adizes-robot-animated"
                />
              </div>
            </section>
          )}
              ...
              <div className="adizes-questions-main">
                ...
                <div className="adizes-questions-list">
                  {currentBlockData?.questions.map((question, questionIndex) => {
                    const colors = currentBlockData?.colors || ['#FF0000', '#FFD700', '#00FF00', '#0000FF'];
                    const borderColor = colors[questionIndex];
                    const typeLabels = ['P', 'A', 'E', 'I'];
                    const typeLabel = typeLabels[questionIndex];
                    ...
                    >
                      <div className="md:hidden flex items-center gap-2">
                        <span className="text-[12px] font-bold" style={{ color: borderColor }}>
                          {typeLabel}
                        </span>
                        <p className="text-[14px] text-[#262626] font-unbounded font-[400]">
                          {question}
                        </p>
                      </div>
                      <div 
                        className="flex justify-between items-center py-[14px] px-[19px] bg-white rounded-[5px]"
          ...
          {showResults &&
            (() => {
              ...
              if (isShowingSavedResult) {
                ...
                // 25-40 баллов = большая буква (ведущий стиль)
                // 20-24 балла = маленькая буква (проявляется в незначительной мере)
                // Меньше 20 баллов = буква не пишется (стиль не развит)
                const getPaeiLetter = (score: number, letter: string) => {
                  if (score >= 25 && score <= 40) return letter.toUpperCase();
                  if (score >= 20 && score <= 24) return letter.toLowerCase();
                  return '';
                };
              ...
              const getDescriptionForType = (type: 'P' | 'A' | 'E' | 'I', score: number) => {
                ...
                if (score >= 25 && score <= 40) {
                  // Сильно выраженная функция (25-40 баллов) - показываем полное описание
                  return typeData.detailedDescription;
                } else if (score >= 20 && score <= 24) {
                  // Умеренно выраженная функция (20-24 балла) - показываем сокращенное описание
              ...
              // Логика определения картинки согласно скриншотам
              const getResultImage = (code: string, scores: { P: number; A: number; E: number; I: number }) => {
                // Подсчитываем количество больших букв
                const uppercaseCount = (code.match(/[A-Z]/g) || []).length;
                const hasP = code.includes('P') || code.includes('p');
                const hasBigP = code.includes('P');
                const hasBigA = code.includes('A');
                const hasBigI = code.includes('I');

                // Администратор: PAei / PA-i / РАе- (когда две большие буквы РА)
                if (hasBigP && hasBigA && uppercaseCount === 2) {
                  return getAssetUrl('_assets/images/paei_answers/Admin.svg');
                }

                // Інтегратор: Рael (когда Р больше по сумме баллов чем І) / pael / pa-l (одна большая буква І, в комбинации с Р)
                // Согласно ТЗ: 
                // 1. Pael - когда P большая, I большая, И P > I по сумме баллов
                // 2. pael / pa-l - когда одна большая I в комбинации с P (P маленькая)
                if (hasBigI && hasP) {
                  // Если обе буквы большие (P и I), проверяем что P > I для формулы Pael
                  if (hasBigP && hasBigI) {
                    // Для Pael нужно чтобы P > I
                    if (scores.P > scores.I) {
                      return getAssetUrl('_assets/images/paei_answers/Inter.svg');
                    }
                  } else {
                    // Если только I большая, а P маленькая (pael / pa-l) - всегда показываем Інтегратора
                    return getAssetUrl('_assets/images/paei_answers/Inter.svg');
                  }
                }

                // Виконавець: Paei / Pa-i / Pae- / P-ei (когда одна большая буква Р без других больших)
                if (hasBigP && uppercaseCount === 1) {
                  return getAssetUrl('_assets/images/paei_answers/Vykon.svg');
                }

                // По умолчанию используем доминирующий тип
                ...
                return getAssetUrl(`_assets/images/paei_answers/${imageMap[dominantType] || 'Per.svg'}`);
              };

              const resultImageUrl = getResultImage(paeiCode, results);
