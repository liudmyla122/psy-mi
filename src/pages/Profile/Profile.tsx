...
export function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const handleTestResultSaved = () => {
      console.log('testResultSaved event received, reloading user info...');
      // Задержка, чтобы сервер успел сохранить данные
      setTimeout(() => {
        loadUserInfo();
      }, 800);
    };
  ...
  const confirmDeleteResult = async () => {
    ...
    try {
      ...
      await axios.delete(`http://localhost:3000/api/test-results/${resultToDelete}`, {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
      });
      ...
      setTestResults((prev) => prev.filter((result) => result.id !== resultToDelete));
      setShowDeleteModal(false);
      setResultToDelete(null);
    } catch (error: any) {
      console.error('Error deleting result:', error);
      const errorMessage = error.response?.data?.detail || error.message || 'Помилка при видаленні результату';
  ...
  const getTestResultsByType = (testType: string) => {
    const filtered = testResults.filter((result) => {
      const resultType = (result.testType || '').toLowerCase().trim();
      const searchType = testType.toLowerCase().trim();
      const matches = resultType === searchType;
      return matches;
    });
    return filtered;
  };
  ...
  const getTestPath = (testType: string) => {
    ...
    switch (testType.toLowerCase()) {
      ...
      case 'mbti':
      case 'мвті':
        basePath = '/test/mbti/result';
        break;
      case 'emotional intelligence':
      case 'emotional-intelligence':
      case 'емоційний інтелект':
        basePath = '/test/emotional-intelligence/result';
        break;
      case 'mbi':
      case 'мві':
        basePath = '/test/mbi';
  ...
  const emotionalIntelligenceResults = getTestResultsByType('emotional-intelligence');
    ...
    <div className="profile-page">
      ...
      <main className="profile-main">
        <div className="profile-container">
          ...
          {enneagramResults.length > 0 && (
            <div className="profile-test-section-wrapper">
              ...
              <div className="profile-test-header">
                ...
                <span className="profile-test-header-details">Подробніше</span>
                <span className="profile-test-header-delete">Видалити</span>
              </div>
                ...
                <div className="profile-test-results-list">
                  {enneagramResults.map((result) => (
                    <div key={result.id} className="profile-test-result-row">
                      ...
                      <button
                        type="button"
                        className="profile-test-delete-button"
          ...
          {/* Секция Емоційний інтелект */}
          {emotionalIntelligenceResults.length > 0 && (
            <div className="profile-test-section-wrapper">
              <h2 className="profile-test-section-title">Емоційний інтелект</h2>
              <div className="profile-test-header">
                <span className="profile-test-header-result">Результат</span>
                <span className="profile-test-header-date">Дата проходження</span>
                <span className="profile-test-header-details">Подробніше</span>
              ...
              <div className="profile-test-section">
                <div className="profile-test-results-list">
                  {emotionalIntelligenceResults.map((result) => (
                    <div key={result.id} className="profile-test-result-row">
                      <span className="profile-test-result-name">{result.result}</span>
                      <span className="profile-test-result-date">
                        {formatDate(result.completedAt)}
      ...
      {showDeleteModal && (
        <div className="profile-delete-modal-overlay" onClick={cancelDeleteResult}>
          <div className="profile-delete-modal-content" onClick={(e) => e.stopPropagation()}>
            <button
            ...
            <div className="profile-delete-modal-body">
              <div className="profile-delete-modal-robot">
                <img src={sadRobotImageUrl} alt="Sad Robot" />
              </div>
              <div className="profile-delete-modal-text-content">
                <h2 className="profile-delete-modal-title">Ви впевнені?</h2>
                <p className="profile-delete-modal-message">
                  Ви точно хочете видалити цей результат? Цю дію неможливо скасувати.
                </p>
                <div className="profile-delete-modal-buttons">
                  <button
                    className="profile-delete-modal-button profile-delete-modal-button-outline"
                    onClick={cancelDeleteResult}
                  >
                    Ні
                  </button>
                  <button
                    className="profile-delete-modal-button profile-delete-modal-button-primary"
